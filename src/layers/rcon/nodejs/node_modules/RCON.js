...
exports.RCON = class RCON {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.socket = net
        ...
        .setTimeout(this.timeout, () => {
          this.socket.destroy()
          reject(
            new Error(`Socket timed out when connecting to [${server}:${port}]`)
          )
        })
        .once('connect', () => {
          this.online = true
  ...
  send(command) {
    if (!this.online || !this.authenticated) {
      ...
      return new Promise((resolve, reject) => {
        const id = Packet.id()
        this.queue.sending.push(
          Packet.write(id, Packet.type['COMMAND'], command)
        )
        this.queue.sending.push(
          Packet.write(id, Packet.type['COMMAND_END'], '')
        )
        this.drain()
          ...
          if (result instanceof Error) {
            ...
            if (result.type !== Packet.type['COMMAND_RES']) {
              ...
              if (result.payload === Packet.payload['COMMAND_END']) {
                const response = this.queue.pending[result.id].payloads.reduce(
